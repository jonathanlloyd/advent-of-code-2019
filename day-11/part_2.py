...
class Robot:
    def __init__(self, programme):
        ...
        # Start panel is painted white
        self._painted_panels[(0, 0)] = 1
    ...
    def render(self):
        white_panels = {coord for coord, color in self._painted_panels.items() if color == 1}

        top_left_x = min([coord[0] for coord in white_panels])
        top_left_y = min([coord[1] for coord in white_panels])
        bottom_right_x = max([coord[0] for coord in white_panels])
        bottom_right_y = max([coord[1] for coord in white_panels])
        ...
        for y in range(height, -1, -1):
            ...
            for x in range(0, width):
                ...
                if (adjusted_x, adjusted_y) in white_panels:
                    row += '#'
                else:
                    row += ' '
            row += '\n'
...
def painting_from_programme(programme):
...
if __name__ == '__main__':
    ...
    result = painting_from_programme(programme)
    print('Painting:', '\n' + result)
